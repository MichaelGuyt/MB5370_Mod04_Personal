...
ggplot() +
  geom_bar(data = data_long, aes(x = Date, y = Count, fill = Type), stat = "identity", position = position_dodge(width = 0.8), width = 1.8, alpha = 0.7) +
  geom_line(data = data_env, aes(x = Date, y = Value/7, color = Variable, group = Variable), size = 0.6) +
  scale_y_continuous(name = "Count (Pupa & Adult)", limits = c(0,7), sec.axis = sec_axis(~ . * 7, name = "Rainfall & Temperature (mm)")) +
  labs(title = "Pupa and Adult counts with rainfall & temperature",
       x = "Date") +
  scale_fill_manual(values = c("Pupa" = "blue", "Adult" = "red")) +
  scale_color_manual(values = c("Rainfall" = "darkgreen", "Temperature" = "orange")) +
  theme_minimal() +
